...
> **关于 sudo**：除 `help` 外，**所有 `afctl` 命令都需 root**——① docker 守护进程默认只允许 root；② `config/env` 是 root 私有(`600`)。afctl 在入口统一拦截：忘加 sudo 会立刻提示 **"请用 root 运行：sudo ./afctl …"**，绝不会再出现某条命令半失败/卡住/谎报的情况。**记法：afctl 一律 sudo。**
...
```bash
sudo ./afctl logs trade      # logs/trade.jsonl —— 交易关键事件（连接/reconcile/触发/成交/撤单/风控）
sudo ./afctl logs audit      # logs/audit.jsonl —— 交易事件 + 采样常规事件（默认 1% 采样）
sudo ./afctl logs engine     # 引擎容器 stdout/stderr —— 异常堆栈、底层库提示
sudo ./afctl logs gateway    # IB Gateway 容器日志 —— 登录与会话状态
```
...
4. 验证与生效：
   ...
   sudo ./afctl logs watchdog   # 查看看门狗
   ```
